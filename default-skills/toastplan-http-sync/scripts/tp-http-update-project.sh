...
if [[ $# -ne 2 ]]; then
  echo "Usage: $0 <project_id> <updates_json>" >&2
  echo "Example: $0 project-123 '{\"linkedGoalId\":\"goal-1\",\"isAiAutonomous\":true}'" >&2
  exit 1
fi
...
' "${PROJECT_ID}" "${UPDATES_JSON}")"

tp_http_call_tool "update_project" "${ARGS_JSON}"
