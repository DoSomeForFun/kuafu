...
// 1. Implement LLM function (swap in OpenAI / Anthropic / local model)
...
// 3. Create Kernel with injected LLM
...
|-------|------|----------|-------------|
| `taskId` | `string` | ✅ | Unique task ID (must be created via `Store.createTask` first) |
| `prompt` | `string` | ✅ | User input prompt |
| `sessionId` | `string` | ✅ | Session ID (keep consistent across turns in the same conversation) |
| `maxSteps` | `number` | ❌ | Max FSM loop steps (default 30) |
| `maxHistory` | `number` | ❌ | Number of history messages to include (default 10) |
| `agentName` | `string` | ❌ | Agent name identifier |
| `retrievedContext` | `any[]` | ❌ | Externally retrieved context blocks |
| `progressSink` | `ProgressSink` | ❌ | Real-time step progress callback |
| `outcomeSink` | `OutcomeSink` | ❌ | Final result callback |

### Store

- `createTask({ id, title, date, notes? })` → create a task
- `getTaskById(taskId)` → fetch a task by ID
- `getMessagesForTask(taskId, branchId?)` → fetch message history
...
- `invokeTool(toolCall)` → execute a tool call
- `getSpecs()` → get available tool specs
